...
function init(){
    ...
    $('#cantidad_articulo').validacion('0123456789');
	mostrarform(false);
    listar();
...
{
	$.post("../ajax/inventario.php?op=4",{id_articulo : id_articulo}, function(data, status)
	{
...
function desactivar(id_articulo)
{
	swal.fire({
		...
		if (result.value) {
			$.post("../ajax/inventario.php?op=2", {id_articulo : id_articulo}, function(e){
				mensaje=e.split(":");
					if(mensaje[0]=="1"){  
						swal.fire(
							'Mensaje de Confirmación',
...
function activar(id_articulo)
{
	swal.fire({
		...
		if (result.value) {
			$.post("../ajax/inventario.php?op=3", {id_articulo : id_articulo}, function(e){
				mensaje=e.split(":");
					if(mensaje[0]=="1"){  
						swal.fire(
							'Mensaje de Confirmación',
