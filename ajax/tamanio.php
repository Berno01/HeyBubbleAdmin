	...
	case '0':
		$rspta=$tamanio->listar();
 		...
 		while ($reg = mysqli_fetch_assoc($rspta)){			
			$data[]=array(
				"1"=>($reg['estado_tamanio']=='0')?'<button class="btn btn-icon btn-hover btn-sm btn-rounded pull-right" onclick="mostrar('.$reg['id_tamanio'].')"><i class="anticon anticon-edit"></i></button>'.
